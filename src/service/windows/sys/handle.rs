...
impl ScHandle {
    ...
    /// # Safety
    ///
    /// The `handle` must be a valid, unclosed service control manager handle.
    pub const unsafe fn new(handle: SC_HANDLE) -> Self {
        ScHandle(handle)
    }

    /// Returns the underlying [`SC_HANDLE`].
    /// For safety reasons, the returned handle SHOULD NOT be kept alive.
    pub const fn raw_handle(&self) -> SC_HANDLE {
        self.0
    }
}
...
impl Drop for ScHandle {
    fn drop(&mut self) {
        unsafe {
            Services::CloseServiceHandle(self.0).expect("failed to close service handle");
        }
    }
}
